...
describe AssignClientToProductService, type: :services do
    ...
    context "active product" do
        it "allow to assign" do
            result = subject.exec(user_id: user.id, product_id: active_product.id, duration: 1.month)
            expect(result.success).to be_truthy
        end

        it "prevent duplicate" do
            subject.exec(user_id: user.id, product_id: active_product.id, duration: 1.month)
            result = subject.exec(user_id: user.id, product_id: active_product.id, duration: 1.month)
    ...
    context "inactive product" do
        it "disallow to assign" do
            result = subject.exec(user_id: user.id, product_id: inactive_product.id, duration: 1.month)
            expect(result.success).to be_falsy
        end
    end
end
